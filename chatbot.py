...
def decoder_rnn(decoder_embedded_input,decoder_embeddings_matrix,encoder_state,num_words,sequence_length,rnn_size,num_layers,word2int,keep_prob,batch_Size):
    with tf.variable_scope("decoding") as decoding_scope:
        ...
        output_function=lambda x:tf.contrib.rnn.layers.fully_connected(x,
                                                                       num_words,
                                                                       None,
                                                                       scope=decoding_scope,
...
def seq2seq_model(inputs,targets,keep_prob,batch_size,sequence_length,answers_num_words,encoder_embedding_size,decoder_embedding_size,rnn_size,num_layers,questionswords2int):
...
for epoch in range(1,epochs+1):
    for batch_index,(padded_questions_in_batch,padded_answers_in_batch) in enumerate(split_into_batches(training_questions,training_answers,batch_size)):
        ...
        batch_time=starting_time-ending_time
        if batch_index % batch_index_check_training_loss==0:
            print('Epoch: {:>3}/{},Batch: {:>4}/{},Training_loss_error: {:>6.3f},Traiing time on 100 batches: {:d} seconds'.format(epoch,
                                                                                                                                   epochs,
                                                                                                                                   batch_index,
        ...
        if batch_index % batch_index_check_validation_loss==0 and batch_index>0:
            ...
            for batch_index_validation,(padded_questions_in_batch,padded_answers_in_batch) in enumerate(split_into_batches(validation_questions,validation_answers,batch_size)):
                batch_validation_loss_error=session.run([optimizer_gradient_clipping,loss_error],{inputs:padded_questions_in_batch,
                                                                                                           targets:padded_answers_in_batch,
                                                                                                           lr:learning_rate,
                                                                                                           sequence_length:padded_answers_in_batch.shape[1],
                                                                                                           keep_prob:1})
...
####### PART-4 - TESTING THE SEQ-2-SEQ MODEL #######
